...
| `SEED_ADMIN_EMAIL`          | With seed         | The super-admin login email to create.                                                                                                                                                                                         |
| `SEED_ADMIN_PASSWORD`       | With seed         | The super-admin password to create (choose a strong one).                                                                                                                                                                      |
...
1. With `SEED_ON_DEPLOY=true` (and `SEED_ADMIN_EMAIL`/`SEED_ADMIN_PASSWORD` set),
   trigger a deploy. The build runs migrations then the seed (super-admin +
   `bos`/`demo` sites + demo content). The seed is **idempotent**.
